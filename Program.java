...
class Program {
    public static void main(String[] args) throws Exception {
        ...
        CreditManager creditManager = new Program().new CreditManager();
        creditManager.Calculate();
        creditManager.Save();

        Customer customer = new Program().new Customer();
        customer.Id = 1;
        customer.City = "Ankara";
        CustomerManager customerManager = new Program().new CustomerManager(customer,
                new Program().new MilitaryCreditManager());
        customerManager.Save();
        customerManager.Delete();
        customerManager.GiveCredit();
        Company company = new Program().new Company();
        company.TaxNumber = "100000";
        company.CompanyName = "Arçelik";
        company.Id = 100;
        CustomerManager customerManager2 = new Program().new CustomerManager(new Program().new Person(),
                new Program().new TeacherCreditManager());
        customerManager2.GiveCredit();
        CustomerManager customerManager3 = new Program().new CustomerManager(new Program().new Company(),
                new Program().new CarCreditManager());
        customerManager3.GiveCredit();

        Person person = new Program().new Person();
